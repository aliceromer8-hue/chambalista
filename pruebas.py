...
# El nombre del archivo es lo primero que ve quien abre la bandeja.
# Antes llevaba la empresa —«...-Harvard-Consultora-Andina-Practicante»—
# y eso no lo hace una persona, lo hace un sistema.
check("el nombre NO lleva la empresa", "Consultora" not in j["nombre"], j["nombre"])
check("ni el nombre de la plantilla", "Harvard" not in j["nombre"], j["nombre"])
check("y se parece a como la gente nombra su CV",
      j["nombre"].endswith(".docx") and "Romero" in j["nombre"] and "-Harvard-" not in j["nombre"],
      j["nombre"])
...
# ---------------------------------------------------------------------
# Dos rastros que delataban la herramienta, y ninguno tenía que ver con
# el contenido del CV —que no se toca: se reordena y se reformula, nunca
# se inventa—. Eran el envoltorio.

import nombres_cv
from harvard_template import documento_en_memoria

_persona = "Alice Nicoll Romero León"
_muestras = {nombres_cv.elegir(_persona, semilla=f"{e}|{p}", puesto=p)
             for e, p in [("Artesco", "Practicante de Marketing"),
                          ("Backus", "Analista Junior"),
                          ("Interbank", "Practicante de Riesgos"),
                          ("Alicorp", "Practicante Comercial"),
                          ("Rimac", "Asistente de Marketing"),
                          ("BCP", "Practicante de Marca"),
                          ("Gloria", "Practicante de Trade"),
                          ("Ripley", "Practicante de Ecommerce")]}
check("vacantes distintas dan nombres distintos", len(_muestras) >= 4,
      f"{len(_muestras)} formas: {sorted(_muestras)[:3]}")
check("la misma vacante da siempre el mismo",
      len({nombres_cv.elegir(_persona, semilla="Artesco|Marketing") for _ in range(8)}) == 1)
check("ninguno nombra a la empresa",
      not any(e.lower() in n.lower() for n in _muestras
              for e in ("artesco", "backus", "interbank", "alicorp", "rimac", "bcp")),
      str(sorted(_muestras)[:2]))
check("ninguno dice «Harvard»", not any("harvard" in n.lower() for n in _muestras))
check("todos llevan el nombre de la persona",
      all("Romero" in n or "Alice" in n for n in _muestras))
check("y todos son nombres de archivo válidos en Windows",
      all(not set(n) & set('<>:"/\|?*') for n in _muestras))

# Sin nombre no puede reventar: un CV sin nombre detectado sigue siendo
# un CV que hay que poder adjuntar.
check("sin nombre, sigue saliendo un archivo", nombres_cv.elegir("").endswith(".docx"))

# Los metadatos. python-docx firma cada archivo como suyo, y eso se ve
# con clic derecho → Propiedades sin abrir el documento.
_buf, _nom = documento_en_memoria(CV, sufijo="Artesco|Marketing", puesto="Marketing")
_crudo = _buf.getvalue()
with zipfile.ZipFile(io.BytesIO(_crudo)) as _z:
    _props = _z.read("docProps/core.xml").decode("utf-8", "replace")
check("el archivo ya no viene firmado por la librería",
      "python-docx" not in _props, _props[:120])
check("la autora es la persona", CV["nombre"] in _props)
check("y la descripción no cuenta cómo se hizo",
      "generated by" not in _props.lower())

# ---------------------------------------------------------------------
titulo("ATS — que la máquina de la empresa sepa leerlo")
# ---------------------------------------------------------------------
# Antes de que un humano vea el CV, lo lee un programa que extrae texto y
# lo mete en campos. Lo que rompe a esos programas está muy estudiado y
# es siempre lo mismo: columnas, tablas, cuadros de texto, la foto, los
# datos de contacto metidos en el encabezado.
#
# El formato Harvard los evita todos por cómo es —una columna, texto
# plano, sin foto—, pero eso es fácil de romper sin querer al retocar la
# plantilla. Por eso está aquí: si alguien mete una tabla para alinear
# algo, esto salta antes de que se manden mil CV ilegibles.

from docx import Document as _Doc

_buf_ats, _ = documento_en_memoria(CV)
_bytes_ats = _buf_ats.getvalue()
_doc_ats = _Doc(io.BytesIO(_bytes_ats))
with zipfile.ZipFile(io.BytesIO(_bytes_ats)) as _z_ats:
    _xml_ats = _z_ats.read("word/document.xml").decode("utf-8", "replace")
    _partes_ats = _z_ats.namelist()
_texto_ats = "\n".join(p.text for p in _doc_ats.paragraphs)

check("una sola columna, sin tablas", len(_doc_ats.tables) == 0, f"{len(_doc_ats.tables)} tablas")
check("sin cuadros de texto, que el ATS no lee", "<w:txbxContent" not in _xml_ats)
check("sin imágenes ni foto", "<w:drawing" not in _xml_ats and "<w:pict" not in _xml_ats)
check("sin encabezado ni pie, que muchos ATS ignoran",
      not [p for p in _partes_ats if "header" in p or "footer" in p])
check("fuente estándar",
      (_doc_ats.styles["Normal"].font.name or "").lower()
      in ("times new roman", "arial", "calibri"), str(_doc_ats.styles["Normal"].font.name))
check("el correo va en el texto, no solo en un hipervínculo",
      CV["contacto"]["email"] in _texto_ats)
check("los títulos de sección se leen en plano",
      all(t in _texto_ats.upper() for t in ("EDUCACIÓN", "EXPERIENCIA")))
